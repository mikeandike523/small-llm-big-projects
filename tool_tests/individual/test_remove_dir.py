...
def run(env: TestEnv, server: MicroServer | None = None):
    ...
    try:
        ...
        cl.check(
            ...
            "removed" in r.lower() or empty_dir in r,
            f"got: {r!r}",
        )
        cl.check(
            "empty dir gone",
            "Empty directory no longer exists after removal",
            not os.path.exists(empty_dir),
            f"path: {empty_dir!r}",
        )
        ...
        cl.check(
            "remove non-empty dir result",
            "Result indicates directory was removed recursively",
            "removed" in r2.lower() or non_empty_dir in r2,
            f"got: {r2!r}",
        )
        cl.check(
            "non-empty dir gone",
            "Non-empty directory no longer exists after recursive removal",
            not os.path.exists(non_empty_dir),
            f"path: {non_empty_dir!r}",
        )
        ...
        cl.check(
            ...
            "Error" in r3 or "not exist" in r3.lower(),
            f"got: {r3!r}",
        )
