...
def add_checks(cl: CheckList, env: TestEnv) -> None:
    ...
    execute_tool("todo_list", {"action": "add_item", "parent_path": "", "text": "parent task"}, env.session_data)
    execute_tool("todo_list", {"action": "add_item", "parent_path": "", "text": "sibling"}, env.session_data)

    # Promote item 1 by adding a child
    r = execute_tool("todo_list", {"action": "add_item", "parent_path": "1", "text": "child A"}, env.session_data)
    d = _j(r)
    cl.check("promotion child path", "First child of promoted item has path '1.1'",
             d.get("item_path") == "1.1", f"got: {r!r}")

    r = execute_tool("todo_list", {"action": "add_item", "parent_path": "1", "text": "child B"}, env.session_data)
    ...
    r = execute_tool("todo_list", {"action": "update_item", "item_path": "1", "text": "renamed group"}, env.session_data)
