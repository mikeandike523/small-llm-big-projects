...
def add_checks(cl: CheckList, env: TestEnv) -> None:
    ...
    r = execute_tool(
        "todo_list",
        {
            ...
            "parent_path": "",
            "before": 2,
            "text": "inserted before 2",
        },
        env.session_data,
    )
    d = _j(r)
    cl.check(
        "add_item before path",
        "Insert before 2 gives item_path='2'",
        d.get("item_path") == "2",
        f"got: {r!r}",
    )
    r = execute_tool(
    ...
    cl.check(
        "add_item before text",
        "Item at position 2 is the newly inserted item",
        r == "inserted before 2",
        f"got: {r!r}",
    )
    ...
    cl.check(
        "add_item after path",
        "Insert after 1 gives item_path='2'",
        d.get("item_path") == "2",
        f"got: {r!r}",
    )
    r = execute_tool(
    ...
    cl.check(
        "add_item after text",
        "Item at position 2 is the after-inserted item",
        r == "inserted after 1",
        f"got: {r!r}",
    )
