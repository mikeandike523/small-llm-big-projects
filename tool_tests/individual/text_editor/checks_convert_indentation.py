...
def add_checks(cl: CheckList, env: TestEnv) -> None:
    ...
    cl.check(
        "spaces to tabs result",
        "Result message mentions indentation conversion",
        "convert" in r.lower() or "indentation" in r.lower(),
        f"got: {r!r}",
    )
    stored = env.session_data["memory"].get("cvt_spaced")
    cl.check(
        "tabs present",
        "Converted content uses tab characters",
        "\t" in stored,
        f"got: {stored!r}",
    )
    cl.check(
        "leading spaces removed",
        "4-space indentation is replaced (no leading spaces)",
        "    pass" not in stored and "    return" not in stored,
        f"got: {stored!r}",
    )
    ...
    execute_tool(
        "text_editor",
        {"action": "convert_indentation", "key": "cvt_tabbed", "to": "spaces"},
        env.session_data,
    )
    stored2 = env.session_data["memory"].get("cvt_tabbed")
    cl.check(
        "spaces present",
        "Converted content uses space indentation",
        "    pass" in stored2,
        f"got: {stored2!r}",
    )
    cl.check(
    ...
    cl.check(
        "custom spaces_per_tab",
        "spaces_per_tab=2 produces 2-space indentation",
        stored3 == "root:\n  child\n",
        f"got: {stored3!r}",
    )
    ...
    cl.check(
        "missing to error",
        "Returns error when 'to' arg is missing",
        r.startswith("Error:"),
        f"got: {r!r}",
    )
