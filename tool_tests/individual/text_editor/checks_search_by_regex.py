...
def add_checks(cl: CheckList, env: TestEnv) -> None:
    ...
    cl.check(
        "match count",
        "Reports number of matches in header",
        "2 match" in r,
        f"got: {r!r}",
    )
    cl.check(
        "match lines present",
        "Matching lines appear in output",
        "alpha" in r and "alpha again" in r,
        f"got: {r!r}",
    )
    cl.check(
        "non-matching excluded",
        "Non-matching line is not in output",
        "beta" not in r and "gamma" not in r,
        f"got: {r!r}",
    )
    ...
    cl.check(
        "no matches",
        "Reports no matches when pattern absent",
        "no matches" in r.lower(),
        f"got: {r!r}",
    )
    ...
    cl.check(
        "invalid regex error",
        "Returns error for malformed regex",
        r.startswith("Error:"),
        f"got: {r!r}",
    )
    ...
    cl.check(
        "empty content",
        "Reports empty when content is empty",
        "empty" in r.lower(),
        f"got: {r!r}",
    )
