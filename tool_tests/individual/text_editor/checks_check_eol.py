...
def add_checks(cl: CheckList, env: TestEnv) -> None:
    ...
    cl.check(
        "lf reported",
        "LF-only content reports LF endings",
        "lf" in r.lower(),
        f"got: {r!r}",
    )
    cl.check(
        "lf uniform",
        "LF-only content reports uniform style",
        "uniform" in r.lower(),
        f"got: {r!r}",
    )
    cl.check(
        "lf no crlf mention",
        "LF-only content does not mention CRLF count",
        "crlf" not in r.lower(),
        f"got: {r!r}",
    )
    ...
    cl.check(
        "crlf reported",
        "CRLF content reports CRLF endings",
        "crlf" in r.lower(),
        f"got: {r!r}",
    )
    cl.check(
        "crlf uniform",
        "CRLF content reports uniform style",
        "uniform" in r.lower(),
        f"got: {r!r}",
    )
    ...
    cl.check(
        "mixed reported",
        "Mixed content reports mixed style",
        "mixed" in r.lower(),
        f"got: {r!r}",
    )
    ...
    cl.check(
        "empty no endings",
        "Empty string reports no line endings",
        "no line endings" in r.lower(),
        f"got: {r!r}",
    )
