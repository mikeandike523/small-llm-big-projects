...
def add_checks(cl: CheckList, env: TestEnv) -> None:
    ...
    cl.check(
        ...
        "Deleted" in r or "deleted" in r.lower(),
        f"got: {r!r}",
    )
    cl.check(
        "delete: key gone",
        "Key is no longer in memory",
        "todelete" not in env.session_data["memory"],
        "key still present",
    )
    ...
    cl.check(
        ...
        "Deleting missing key returns error string with 'does not exist'",
        "Failed" in r2 and "does not exist" in r2,
        f"got: {r2!r}",
    )
