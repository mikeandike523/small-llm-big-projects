...
def run(env: TestEnv, server: MicroServer | None = None):
    ...
    try:
        ...
        cl.check(
            ...
            "created" in r.lower() or target_path in r,
            f"got: {r!r}",
        )
        cl.check(
            "file exists",
            "File actually exists on disk after creation",
        ...
        cl.check(
            "file is empty",
            "Newly created file is empty (touch semantics)",
            contents == "",
        ...
        cl.check(
            "create_parents result",
            "Result indicates file was created",
            "Error" not in r4,
            f"got: {r4!r}",
        )
        cl.check(
            "create_parents file exists",
            "File actually exists when create_parents=True",
            os.path.isfile(parents_path),
            f"path: {parents_path!r}",
        )
        ...
        with open(content_path, "r", encoding="utf-8") as f:
            written = f.read()
        cl.check(
            "initial_content written",
            "File contains the specified initial content",
            written == "hello content",
            f"got: {written!r}",
        )
